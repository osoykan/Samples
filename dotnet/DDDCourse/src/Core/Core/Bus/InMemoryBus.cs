...
namespace Core.Bus
{
    public class InMemoryBus : IBus
    {
        ...
        public Task Send<T>(T command) where T : Command
        {
            ...
            {
                if (handlers.Count != 1) throw new InvalidOperationException("Command Handler cannot be more than one");

                handlers.First()(command);
            }
            else throw new InvalidOperationException("no handler registered");
        ...
        {
            if (!_routes.TryGetValue(typeof(T), out List<Action<IMessage>> handlers)) return Task.CompletedTask;
